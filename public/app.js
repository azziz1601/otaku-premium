document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadGenres() {
    try {
      const data = await window.scraper.fetchGenreList();
      genresContainer.innerHTML = data.map(g => 
  ...
  async function loadHome(page = 1) {
    ...
    try {
      const data = await window.scraper.fetchAnimeList(page);
      let html = data.map(createCardHTML).join('');
      if(page === 1) {
  ...
  async function loadSearch(query, page = 1) {
    ...
    try {
      const data = await window.scraper.fetchSearch(query); 
      let html = data.map(createCardHTML).join('');
  ...
  async function loadDetail(url) {
    ...
    try {
      const d = await window.scraper.fetchAnimeDetail(url);
  ...
  async function loadEpisode(url) {
    ...
    try {
      const d = await window.scraper.fetchEpisodeDetail(url);
      // Save History
  ...
  async function loadGenreDetail(url, title, page = 1) {
    ...
    try {
      const data = await window.scraper.fetchAnimeByGenre(url, page);
      let html = data.map(createCardHTML).join('');
      if (data.length === 0 && page === 1) {
          ...
          if (page === 1) {
            ...
            window.genreLastItems = data.map(i => i.title).join();
          } else {
            const currentItems = data.map(i => i.title).join();
            if (currentItems === window.genreLastItems || data.length === 0) {
  ...
  async function loadSchedule() {
    ...
    try {
      const data = await window.scraper.fetchSchedule();
      const scheduleHTML = data.map(dayInfo => `
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    searchTimeout = setTimeout(async () => {
      ...
      try {
        const data = await window.scraper.fetchSearch(q);
        if (data.length > 0) {
          liveSearchResults.innerHTML = data.slice(0,5).map(a => `
